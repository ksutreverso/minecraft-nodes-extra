...
public object FlagWar {
    ...
    // coord index for flag blocks (world uid:x:y:z) -> Attack
    internal val flagBlockIndex: ConcurrentHashMap<String, Attack> = ConcurrentHashMap()
    private fun flagKey(b: Block): String = "${b.world.uid}:${b.x}:${b.y}:${b.z}"
    ...
    internal fun beginAttack(attacker: UUID, attackingTown: Town, chunk: TerritoryChunk, flagBase: Block): Result<Attack> {
        ...
        // determine if this territory is in an active warzone
        val isWarzone = WarzoneManager.isActive() && WarzoneManager.contains(territory.id)

        // if war is disabled, only allow attacks in active warzone territories
        if (!FlagWar.enabled && !isWarzone) {
            return Result.failure(ErrorNotEnemy)
        }

        // check chunk has a town
        // allow no-owner territories only for warzones
        if (territoryTown === null && !isWarzone) {
            return Result.failure(ErrorNotEnemy)
        }
        ...
        // - in war: must be an enemy target
        // - in warzone: allow any town to attack/capture (enemy check bypassed)
        val targetAllowed = if (isWarzone) true else chunkIsEnemy(chunk, territory, attackingTown)
        if (targetAllowed) {
    ...
    internal fun createAttack(
        ...
        val targetName = territory.town?.name ?: "Warzone"
        val progressBar = Bukkit.getServer().createBossBar("Attacking $targetName at ($flagBaseX, $flagBaseY, $flagBaseZ)", BarColor.YELLOW, BarStyle.SOLID)
        ...
        // flag setup:
        // - obsidian = the “cut the cord” button (break it to cancel)
        // - torch = a little “you are here” marker
        flagBlock.setType(Material.OBSIDIAN)
        ...
        FlagWar.flagBlockIndex.put(flagKey(flagBlock), attack)
    ...
    internal fun createAttackBeacon(
        ...
        try {
            ...
            // Fall back to vanilla setType path below
        }

        if (!usedFastEdit) {
            for (block in skyBeaconWireframeBlocks) {
                if (createFrame) {
            ...
            for (block in skyBeaconColorBlocks) {
                if (createColor) {
                    block.setType(SKY_BEACON_BLOCK)
                }
            }
        }
    }
    ...
    internal fun cancelAttack(attack: Attack) {
        ...
        FlagWar.flagBlockIndex.remove(flagKey(attack.flagBlock))
    ...
    internal fun finishAttack(attack: Attack) {
        ...
        // capture rules in one-liners:
        // - normal war: only the core chunk flips the whole territory
        // - warzone: any chunk capture flips the whole territory
        val isWarzone = WarzoneManager.isActive() && WarzoneManager.contains(chunk.territory.id)
        if (isWarzone || chunk.coord == chunk.territory.core) {
            val territory = chunk.territory
            val territoryTown = territory.town
            ...
            // captured enemy territory
                ...
                val fromName = territory.town?.name ?: "Warzone"
                Message.broadcast("${ChatColor.DARK_RED}[War] ${attacker?.name} captured territory (id=${territory.id}) from $fromName!")
            }
        }
