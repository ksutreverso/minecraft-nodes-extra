...
import phonon.nodes.constants.ErrorNoTerritory
...
import phonon.nodes.constants.INTERACTIVE_BLOCKS
...
public class NodesWorldListener : Listener {
    ...
    public fun onBlockBreak(event: BlockBreakEvent) {
        ...
        // quick escape hatch:
        // if you broke the obsidian flag block, pull the plug on this attack
        // (we do this before any no-build/perm checks so it always works)
        if (Nodes.war.enabled || WarzoneManager.isActive()) {
            val directAttack = FlagWar.blockToAttacker[block]
                ?: FlagWar.flagBlockIndex["${block.world.uid}:${block.x}:${block.y}:${block.z}"]
            if (directAttack != null) {
                if (!Config.allowBreakingAlliesFlags && player.uniqueId != directAttack.attacker) {
                    val relationship = getRelationshipOfPlayerToTown(player, directAttack.town)
                    if (relationship in setOf(DiplomaticRelationship.NATION, DiplomaticRelationship.ALLY, DiplomaticRelationship.TOWN)) {
        ...
        // fallback: if direct map lookup missed, try matching by the attack for this chunk and xyz
        if ((Nodes.war.enabled || WarzoneManager.isActive()) && territoryChunk?.attacker !== null) {
            val possAttack = FlagWar.chunkToAttacker.get(territoryChunk.coord)
            if (possAttack != null &&
                block.world == possAttack.flagBlock.world &&
                block.x == possAttack.flagBlock.x &&
                block.y == possAttack.flagBlock.y &&
                block.z == possAttack.flagBlock.z
            ) {
                if (!Config.allowBreakingAlliesFlags && player.uniqueId != possAttack.attacker) {
                    val relationship = getRelationshipOfPlayerToTown(player, possAttack.town)
                    if (relationship in setOf(DiplomaticRelationship.NATION, DiplomaticRelationship.ALLY, DiplomaticRelationship.TOWN)) {
                        event.setCancelled(true)
                        Message.error(player, "[War] Cannot break ally war flags")
                        return
        ...
        // no-build bubble around flags:
        // hands off blocks near the flag while the fight is on
        // (fence/torch breaking doesn’t cancel anything)
        if ((Nodes.war.enabled || WarzoneManager.isActive()) && territoryChunk?.attacker !== null) {
            val attack = FlagWar.chunkToAttacker.get(territoryChunk.coord)!!

            // handle other blocks within no build region
    ...
    public fun onBlockPlace(event: BlockPlaceEvent) {
                ...
                // check if this is flag placement
                    ...
                    if (resident !== null) {
                        ...
                        if (town !== null) {
                            ...
                            if (result.isSuccess) {
                                val attackedTown = territoryChunk.territory.town
                                if (attackedTown != null) {
                                    if (attackedTown === town) {
                                        Message.broadcast("${ChatColor.DARK_RED}[War] ${event.player.name} is liberating ${attackedTown.name} at (${block.x}, ${block.y}, ${block.z})")
                                    } else {
                                        Message.broadcast("${ChatColor.DARK_RED}[War] ${event.player.name} is attacking ${attackedTown.name} at (${block.x}, ${block.y}, ${block.z})")
                                    }
                                } else {
                                    Message.broadcast("${ChatColor.DARK_RED}[War] ${event.player.name} placed a war flag in warzone at (${block.x}, ${block.y}, ${block.z})")
                                }
                            } else {
        ...
        if (resident !== null) {
            ...
            // ignore if war or warzone enabled and item in hand is a flag material
            if ((Nodes.war.enabled || WarzoneManager.isActive()) && Config.flagMaterials.contains(block.type)) {
                return
            }
        }
